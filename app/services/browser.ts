...
const DebugBrowser: IBrowser = {
  ...
  openWindow: (tabs: INewTab[]) => {
    const maxTabId = Math.max(0, ...DEBUG_WINDOWS.map(({ tabs }) => Math.max(...tabs.map(({ id }) => id)) || 0));
    const windowId = DEBUG_WINDOWS.reduce((acc, x) => Math.max(acc, x.id), 0) + 1;
    DEBUG_WINDOWS.push({
      id: windowId,
      focused: false,
      tabs: tabs.map((tab, i) => ({
  ...
  openTabs: (tabs: INewTab[]) => {
    ...
    if (activeWindow) {
      const maxTabId = Math.max(0, ...DEBUG_WINDOWS.map(({ tabs }) => Math.max(0, ...tabs.map(({ id }) => id)) || 0));
      activeWindow.tabs.push(...tabs.map((tab, i) => ({
        ...tab,
        id: maxTabId + i + 1,
        windowId: activeWindow.id,
      })));
      if (tabs.length > 0) {
        DebugBrowser.activateTab(activeWindow.tabs[activeWindow.tabs.length - 1].id);
      }
    } else {
...
const ProductionBrowser: IBrowser = {
  ...
  closeWindow: (windowId: number) => { return browser.windows.remove(windowId).then(() => undefined) },
  openWindow: (tabs: INewTab[]) => {
