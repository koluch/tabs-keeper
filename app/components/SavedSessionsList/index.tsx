  ...
  renderActiveSession(activeSavedSession: ISavedSession | null) {
    if (activeSavedSession) {
      const handleReopenAllWindows = () => {
        activeSavedSession.windows.forEach(window =>
          this.props.onReopenWindow(window.tabs)
        );
      };
  ...
  render() {
    ...
    return (
      <div className={styles.root}>
        {this.props.savedSessionHeaders.length === 0 && (
          <div>No saved sessions</div>
        )}
        {[...this.props.savedSessionHeaders]
          .reverse()
          .map((sessionHeader: ISavedSessionHeader) => {
            const isActive = activeSavedSessionHeader
              ? activeSavedSessionHeader.id === sessionHeader.id
              : false;
            ...
            return [
              <SavedSessionListItem
                ...
                isActive={isActive}
                onClick={() => {
                  this.props.onSelectSession(isActive ? null : sessionHeader);
                }}
                onClickDelete={() => {
                  this.props.onDeleteSession(sessionHeader.id);
                }}
                savedSessionHeader={sessionHeader}
              />,
              isActive
                ? this.renderActiveSession(this.props.activeSavedSession)
                : null
            ];
          })}
      </div>
